...
const bit<8> LOOKUP_IPSET   = 0x1;
const bit<8> LOOKUP_RULES   = 0x3;

#define ACL_MAX_COUNTERS            1024
#define ACL_IP_SET_APPLIED          10
#define ACL_IP_PORT_APPLIED         20
#define ACL_BLOCKED                 80
#define ACL_DENIED                  90
#define ACL_ING_IP_PORT_MISMATCH    91
#define ACL_EG_IP_PORT_MISMATCH     92
	...
	inout pna_main_output_metadata_t ostd)
{
	...
	table set_meta_udp {
        ...
        action acl_rip_allowed (RuleMatchMask_t ipset_matched_mask) {
            meta.ipset_check_result = ipset_matched_mask;
        }

        table acl_src_ipset_table {
            key = {
                meta.acl_pol_id : exact;
            ...
            actions = {
                acl_deny;
                acl_rip_allowed;    /* Remote IP address allowed */
            }
            const default_action = acl_deny;
        }

        table acl_dst_ipset_table {
            key = {
                meta.acl_pol_id : exact;
	...
	apply {
		...
		create_reverse_ct = false;
                ...
                {
                    acl_srcip_proto_table.apply();
                    if ((meta.acl_status & LOOKUP_IPSET) == LOOKUP_IPSET)
                    {
                        acl_dst_ipset_table.apply();
                    }
                    if (meta.acl_status == LOOKUP_RULES)
                    {
                        switch (meta.range_check_ref) {
                            CHECK_TCP_DST_PORT_RANGE: {
                                if (hdr.tcp.isValid()) {
                                    tcp_dst_port_rc_table.apply();
                                }
                            }

                            CHECK_UDP_DST_PORT_RANGE: {
                                if (hdr.udp.isValid()) {
                                    udp_dst_port_rc_table.apply();
                                }
                            }

                            CHECK_SCTP_DST_PORT_RANGE: {
                                if (hdr.sctp.isValid()) {
                                    sctp_dst_port_rc_table.apply();
                                }
                            }

                            CHECK_UDPL_DST_PORT_RANGE: {
                                if (hdr.udpl.isValid()) {
                                    udpl_dst_port_rc_table.apply();
                                }
                            }
                ...
                if ((hdr.ipv4.isValid()) &&
                    (meta.direction == PNA_Direction_t.NET_TO_HOST))
                {
                    acl_dstip_proto_table.apply();
                    if ((meta.acl_status & LOOKUP_IPSET) == LOOKUP_IPSET)
                    {
                        acl_src_ipset_table.apply();
                    }
                    if (meta.acl_status == LOOKUP_RULES)
                    {
                        switch (meta.range_check_ref) {
